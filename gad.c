...
void cloneFndr_m(founder fndr, founder tmp, int steps){
	int i;
	if(tmp==NULL){
		tmp = malloc(sizeof(gad));
		tmp->aa1=malloc(param.aaLength*sizeof(int));
...
void start_n(founder f, int steps){
	int i, size;
...
void start1_n(founder f, int steps){
	int i, size;
